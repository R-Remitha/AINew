...
const USR = () => {
  ...
  useEffect(() => {
    ...
    axios.get('dbsentencetype/')
    .then(response => setsentenceTData(response.data))
    .catch(error => console.log(error));

    axios.get('dbspeakersview/')
    .then(response => setspeakersviewData(response.data))
    .catch(error => console.log(error));

    axios.get('dbdeprelation/')
    .then(response => setDepRelData(response.data))
    .catch(error => console.log(error));
 ...
 const saveDownload=()=>{
  ...
  return (
    ...
    <label htmlFor="status">Status:</label><input type="text" id="status" value={reviewStatus} readOnly/>

    <Button variant="contained" onClick={saveDownload} disabled={reviewStatus === "In Review"}>Save & Download</Button>
    </div>
      ...
      <tr>
        <div className='headerdiv'><th>Concept</th></div>
        {
          selectedData.Concept.map((item,i) => {
            return <td><div className="headerdiv3"><input type="text" value={item} onChange={(event) => handleChange(event, 'Concept', i)}/></div></td>
          }
  ...
  <div className='headerdiv'><th>Index</th></div>
  {
    Array.from({length: selectedData.Concept.length}, (_, i) => i+1).map((item,i) => {
      return <td><div className="headerdiv3"><input type='text' value={item} onChange={(event) => handleChange(event, 'Index', i)} disabled='True'/></div></td>
    })
  }
</tr>
      ...
      <tr>
        <div className='headerdiv'><th>G-N-P</th></div>
        {
           selectedData.GNP.map((item,i) => {
            return <td><div className="headerdiv3"><input type='text' value={item} onChange={(event) => handleChange(event, 'GNP', i)}/></div></td>
          }
      ...
      <tr>
        <div className='headerdiv'><th>Discourse</th></div>
        {
          selectedData.Discourse.map((item,i) => {
            return <td><div className="headerdiv3"><input type='text' value={item} onChange={(event) => handleChange(event, 'Discourse', i)}/></div></td>
          }
      ...
      <tr>
        <div className='headerdiv'><th>Scope</th></div>
        {
          selectedData.Scope.map((item,i) => {
            return <td><div className="headerdiv3"><input type='text' value={item} onChange={(event) => handleChange(event, 'Scope', i)}/></div></td>
          }
      ...
      <tr>
        <div className='headerdiv'><th>Construction</th></div>
        {
          selectedData.Construction.map((item,i) => {
            return <td colSpan={selectedData.Concept.length}><div className="headerdiv3"><input type='text' value={item} onChange={(event) => handleChange(event, 'Construction', i)}/></div></td>
          }
